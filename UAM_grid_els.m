function grid_els = UAM_grid_els(pt1,pt2,x_min,y_min,x_max,y_max,grid_x,M,N)
...
im = zeros(M,N);
r1 = max(1,floor(M - (((pt1(2)-y_min)/(y_max-y_min))*(M-1) + 1) + 1));
c1 = max(1,floor(((pt1(1)-x_min)/(x_max-x_min))*(N-1) + 1));
pixel1 = [r1,c1];
r2 = max(1,floor(M - (((pt2(2)-y_min)/(y_max-y_min))*(M-1) + 1) + 1));
c2 = max(1,floor(((pt2(1)-x_min)/(x_max-x_min))*(N-1) + 1));
pixel2 = [r2,c2];
