function solution  = UAM_A_star(G,initial_state,goal_state,h_name)
% UAM_A_star - A* algorithm for UAS
% On input:
%     G (graph structure): graph definition
%      .vertexes (nx6 array): 3D endpoints of each lane
%      .edges (px2 array): (directed) edges between vertexes
%      .weights (px1 vector): weight on each edge
...
% Call:
%     [so,no] = UAM_A_star(G,3,9,'UAM_A_star_Man')
%     so =  [3,6,9]
%
...
%     T. Henderson
%     UU
%     Fall 2019; modified Spring 2020
%
